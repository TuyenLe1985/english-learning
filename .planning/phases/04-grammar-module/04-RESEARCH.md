...
1. **areaSlug routing strategy for weak-questions endpoint**
...
2. **Seed question count distribution per topic**
...
|------------|------------|-----------|---------|----------|
| Node.js | All JS tasks | Yes | v22.22.2 | — |
| pnpm | Package install | Yes | 9.15.9 | — |
| Docker | DB + services | Yes | 29.4.2 | — |
| PostgreSQL (via Docker) | Prisma migration | Available via docker compose | 16.x | — |
| @dnd-kit/core | DragAndDropExercise | Not yet installed | 6.3.1 (latest) | No fallback — required by D-10 |
| @dnd-kit/sortable | DragAndDropExercise | Not yet installed | 10.0.0 (latest) | No fallback — install alongside core |

**Missing dependencies with no fallback:**
- `@dnd-kit/core` and `@dnd-kit/sortable` must be installed in `apps/web` before DragAndDropExercise can be built. Wave 0 install task required.
...
| Config (api) | `apps/api/vitest.config.ts` — `environment: 'node'`, includes `src/**/*.spec.ts` |
| Config (web) | `apps/web/vitest.config.ts` — `environment: 'jsdom'`, includes `src/**/*.test.{ts,tsx}`, setupFiles: `test-setup.ts` |
...
|--------|----------|-----------|-------------------|-------------|
| GRAM-01 | `getAreas()` returns 10 areas with correct slugs | unit (service) | `pnpm --filter @repo/api test -- grammar.service.spec` | No — Wave 0 |
| GRAM-01 | `getTopicsByArea(slug)` returns correct topics | unit (service) | `pnpm --filter @repo/api test -- grammar.service.spec` | No — Wave 0 |
| GRAM-02 | Lesson detail includes explanation + examples | unit (service) | `pnpm --filter @repo/api test -- grammar.service.spec` | No — Wave 0 |
| GRAM-03 | MultipleChoiceExercise renders options and calls onCorrect/onIncorrect | component | `pnpm --filter @repo/web test -- multiple-choice-exercise.test` | No — Wave 0 |
| GRAM-03 | FillInTheBlankExercise validates case-insensitive answer | component | `pnpm --filter @repo/web test -- fill-in-the-blank-exercise.test` | No — Wave 0 |
| GRAM-03 | DragAndDropExercise renders word bank and blank slots | component | `pnpm --filter @repo/web test -- drag-and-drop-exercise.test` | No — Wave 0 |
| GRAM-04 | `completeSession()` creates GrammarAttempt rows | unit (service) | `pnpm --filter @repo/api test -- grammar.service.spec` | No — Wave 0 |
| GRAM-04 | `completeSession()` upserts GrammarProgress with correct masteryPct | unit (service) | `pnpm --filter @repo/api test -- grammar.service.spec` | No — Wave 0 |
| GRAM-05 | Seed script creates ≥20 questions per topic | integration (seed) | Manual count after `pnpm db:seed` | No — Wave 0 |
| GRAM-06 | `getWeakQuestions()` returns only questions with most-recent isCorrect=false | unit (service) | `pnpm --filter @repo/api test -- grammar.service.spec` | No — Wave 0 |

### Sampling Rate

- **Per task commit:** `pnpm --filter @repo/api test` (api tasks) or `pnpm --filter @repo/web test` (web tasks)
- **Per wave merge:** `pnpm turbo test`
- **Phase gate:** Full suite green before `/gsd:verify-work`

### Wave 0 Gaps

- [ ] `apps/api/src/grammar/grammar.service.spec.ts` — covers GRAM-01, GRAM-02, GRAM-04, GRAM-06
- [ ] `apps/web/src/components/grammar/exercises/multiple-choice-exercise.test.tsx` — covers GRAM-03
- [ ] `apps/web/src/components/grammar/exercises/fill-in-the-blank-exercise.test.tsx` — covers GRAM-03
- [ ] `apps/web/src/components/grammar/exercises/drag-and-drop-exercise.test.tsx` — covers GRAM-03
- [ ] No new framework config needed — both vitest configs exist and cover new file paths
...
|---------------|---------|-----------------|
| V2 Authentication | Yes | JwtAuthGuard on all grammar endpoints (established pattern) |
| V3 Session Management | No | Grammar module is stateless server-side; session handled by Auth.js |
| V4 Access Control | Yes | userId always sourced from JWT payload, never request body (T-03-03) |
| V5 Input Validation | Yes | Zod `GrammarSessionCompleteSchema.parse(body)` in session complete endpoint |
| V6 Cryptography | No | Grammar module handles no cryptographic operations |

### Known Threat Patterns for This Stack

| Pattern | STRIDE | Standard Mitigation |
|---------|--------|---------------------|
| userId injection via POST body | Tampering | userId MUST come from `req.user.userId` (JWT payload) only — never from DTO body |
| Mass assignment on GrammarAttempt | Tampering | Only `questionId`, `isCorrect`, `userAnswer` accepted from client; `userId`, `attemptedAt` set server-side |
| Unauthenticated access to grammar endpoints | Elevation of Privilege | `@UseGuards(JwtAuthGuard)` on every controller method |
| Invalid session complete body (malformed JSON) | Denial of Service | Zod schema parse; NestJS global `ValidationPipe` handles malformed JSON with 400 |
| Attempt injection for another user's topic | Tampering | GrammarProgress upsert uses userId from JWT — attacker cannot update another user's progress |
...
- `packages/database/prisma/schema.prisma` — verified all 6 Phase 4 grammar models, exact field names, constraints, and missing slug fields
- `apps/api/src/vocabulary/vocabulary.controller.ts` — verified controller pattern, JwtAuthGuard usage, AuthenticatedRequest type, endpoint declaration order
- `apps/api/src/vocabulary/vocabulary.service.ts` — verified PrismaService injection pattern, Prisma query patterns, NotFoundException handling
- `apps/web/src/components/vocabulary/practice-session.tsx` — verified session orchestrator pattern, state management, batch submit on completion
- `apps/web/src/components/vocabulary/category-card.tsx` — verified card component pattern for area grid
- `apps/web/src/lib/api-client.ts` — verified fetchWithAuth pattern for relay routes
- `apps/web/src/app/(dashboard)/vocabulary/page.tsx` — verified Server Component + getSessionToken() fetch pattern
- `apps/web/src/app/api/vocabulary/categories/route.ts` — verified Next.js relay route pattern
- `apps/web/src/app/api/vocabulary/session/complete/route.ts` — verified POST relay route pattern
- `packages/shared/src/vocabulary.dto.ts` — verified Zod DTO pattern to follow for grammar DTOs
- `apps/api/src/app.module.ts` — verified AppModule structure for GrammarModule registration
- `apps/api/src/vocabulary/vocabulary.module.ts` — verified NestJS module pattern
- `apps/api/src/auth/jwt-auth.guard.ts` — verified JwtAuthGuard implementation
- `.planning/phases/04-grammar-module/04-CONTEXT.md` — all locked decisions
- `.planning/phases/04-grammar-module/04-UI-SPEC.md` — pre-approved component contracts, dnd-kit specs
- `npm view @dnd-kit/core`, `npm view @dnd-kit/sortable` — verified package versions 6.3.1 and 10.0.0

### Secondary (MEDIUM confidence)

- dnd-kit GitHub repository (github.com/clauderic/dnd-kit) — author identity, package legitimacy, no postinstall scripts confirmed
- `apps/web/package.json`, `apps/api/package.json` — verified all existing dependencies and their pinned versions

### Tertiary (LOW confidence)

- None — all claims verified directly from codebase files or npm registry
...
- Standard stack: HIGH — all packages verified via package.json; dnd-kit verified on npm registry
- Architecture: HIGH — all patterns read directly from existing implemented Phase 3 files
- Schema gap: HIGH — verified by direct read of schema.prisma; GrammarArea and GrammarLesson confirmed missing slug fields
- dnd-kit legitimacy: MEDIUM — package confirmed on npm, source repo verified, no postinstall scripts; slopcheck could not run
- Pitfalls: HIGH — all pitfall patterns observed directly in existing codebase (route collision from vocabulary, upsert from SRS pattern, PointerSensor from UI-SPEC)

**Research date:** 2026-06-13
**Valid until:** 2026-07-13 (stable stack — all patterns anchored in existing codebase)
